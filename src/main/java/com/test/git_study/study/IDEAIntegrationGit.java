...
//不然的话会发现什么都没有被合并到，因为合并在这里的意思可以理解为把修改过东西的分支的内容合并到当前这个分支上
//那如果是把没有修改过内容的分支合并当当前分支上，那也就是根本没有合并什么内容了

//10.解决冲突(10ResolveTheConflict.png)简单来说就是同一处地方有不同的修改就会产生冲突
...
//解决步骤
//提示冲突之后可以看到merge按钮，点进去，可以看到三个地方，第一个是当前分支内容，第二个是没有冲突的内容，第三个是要合并过来分支的内容
//然后冲突的内容分别有绿色的和红色进行高亮显示（绿色代表可以直接添加进没有冲突的内容不会产生冲突，红色代表两边都是添加的同一位置内容，会产生冲突，所以此时都通过手动解决这个冲突）
//然后右侧/左侧分别有两个按钮 x 和 >>/<< 也就是删除内容或者把内容添加到中间内容的响应位置
//最后点击accept就可以进行合并了
